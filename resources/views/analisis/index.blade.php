...
<div id="floatingNavAnalisis">
    <button id="toggleFloatingAnalisis" class="floating-toggle-analisis half-oval-btn-analisis" type="button" aria-label="Buka daftar grafik/tabel">
                    ...
                    <div class="filter-col">
                        ...
                        <select class="form-select" name="kegiatan_id" id="kegiatanSelect">
                            ...
                            @foreach ($kegiatans as $kegiatan)
                                <option value="{{ $kegiatan->kegiatan_id }}" {{ request('kegiatan_id') == $kegiatan->kegiatan_id ? 'selected' : '' }}>
                    ...
                    <div class="filter-col">
                        ...
                        <select class="form-select" name="pangkat_jabatan_id" id="pangkatSelect">
                            ...
                            @foreach ($pangkatJabatans as $pangkat)
                                <option value="{{ $pangkat->pangkat_jabatan_id }}" {{ request('pangkat_jabatan_id') == $pangkat->pangkat_jabatan_id ? 'selected' : '' }}>
                    ...
                    <div class="filter-col">
                        ...
                        <select class="form-select" name="jenis_ptk_id" id="jenisPtkSelect">
                            ...
                            @foreach ($jenisPtkList as $jenis)
                                <option value="{{ $jenis->jenis_ptk_id }}" {{ request('jenis_ptk_id') == $jenis->jenis_ptk_id ? 'selected' : '' }}>
                                    {{ $jenis->jenis_ptk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="filter-col">
                        ...
                        <select class="form-select" name="kota_id" id="kotaSelect">
                            ...
                            @foreach ($kotas as $kota)
                                <option value="{{ $kota->kota_id }}" {{ request('kota_id') == $kota->kota_id ? 'selected' : '' }}>
                                    {{ $kota->nama_kota }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="filter-col">
                        ...
                        <select class="form-select" name="jenjang_pendidikan_id" id="jenjangPendidikanSelect">
                            ...
                            @foreach ($jenjangPendidikanList as $jenjang)
                                <option value="{{ $jenjang->jenjang_pendidikan_id }}" {{ request('jenjang_pendidikan_id') == $jenjang->jenjang_pendidikan_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="chart-container" id="sec-level-ptk">
                            <div class="chart-title">
                                ...
                                <small class="text-muted ms-2">(Level terendah yang dicapai PTK di semua sub indikator)</small>
                            </div>
                                            ...
                                            @foreach ($analisisData['modus_per_kota'] as $kota)
                                                @if (!empty($kota['sub_indikator_modus']))
                                                    @foreach ($kota['sub_indikator_modus'] as $index => $sub)
                                                        @php
                                                            $lc = $levelColors[$sub['modus_level'] ?? 2] ?? '#17a2b8';
                                                            $ln = $levelNames[$sub['modus_level'] ?? 2] ?? 'Penerapan';
                                                        @endphp
                                                        <tr>
                                                            @if ($index === 0)
                                                                @if (request('kota_id'))
                                                            ...
                                                            <td>
                                                                <span class="badge-level" style="background-color:{{ $lc }}; color:#fff;">
                                                                    Level {{ $sub['modus_level'] }} ({{ $ln }})
                                                                </span>
                                                            </td>
                        ...
                        <div class="col-12">
                            <div class="table-card" id="sec-gap">
                                <div class="chart-title">
                                    ...
                                    <small class="text-muted ms-2">(Hanya menampilkan sub indikator yang belum mencapai level kompetensi)</small>
                                </div>
                                        ...
                                        @foreach ($analisisData['rekomendasi_gap_per_jenjang'] as $idx => $jenjang)
                                            ...
                                            <li class="nav-item" role="presentation">
                                                <button
                                                    ...
                                                    id="tab-{{ $slug }}"
                                                    data-bs-toggle="tab"
                                                    data-bs-target="#content-{{ $slug }}"
                                                    type="button"
                                                    role="tab">
                                                    {{ $jenjang['jenjang_jabatan'] }}
                                                    <span class="badge bg-danger ms-1">{{ count($jenjang['rekomendasi'] ?? []) }}</span>
                                                </button>
                                            </li>
                                        @endforeach
                                    </ul>
                                ...
                                <div class="tab-content" id="jenjangTabContent">
                                    @foreach ($analisisData['rekomendasi_gap_per_jenjang'] as $idx => $jenjang)
                                        @php $slug = \Illuminate\Support\Str::slug($jenjang['jenjang_jabatan'] ?? 'jenjang'); @endphp
                                        <div class="tab-pane fade {{ $idx == 0 ? 'show active' : '' }}" id="content-{{ $slug }}" role="tabpanel">
                                                    ...
                                                    <table class="table table-bordered modus-table">
                                                        <thead>
                                                            <tr>
                                                                <th width="3%">#</th>
                                                                <th>Sub Indikator</th>
                                                                <th width="8%" class="text-center">Level Dicapai</th>
                                                        ...
                                                        <tbody>
                                                            ...
                                                            @foreach ($jenjang['rekomendasi'] as $rek)
                                                                @if (!empty($rek['detail_gap']))
                                                                    @php
                                                                        $jumlahPtKSubIndikator = $rek['total_ptk_sub_indikator'] ?? 0;
                                                                        $percentage = $jenjangTotalPtk > 0 ? round(($jumlahPtKSubIndikator / $jenjangTotalPtk) * 100, 1) : 0;
                                                                        $progressClass = $percentage >= 30 ? 'bg-danger' : ($percentage >= 15 ? 'bg-warning' : 'bg-info');
                                                                    @endphp
                                                                    ...
                                                                    @foreach ($rek['detail_gap'] as $gapIndex => $gap)
                                                                        <tr>
                                                                            ...
                                                                            <td class="text-center">
                                                                                ...
                                                                                @if ($gapLevel > 0)
                                                                                    <span class="badge bg-danger">+{{ $gapLevel }}</span>
                                                                                @else
                                                                            ...
                                                                            @if ($gapIndex === 0)
                                                                                <td rowspan="{{ count($rek['detail_gap']) }}" class="text-center" style="vertical-align: middle;">
                                                                                ...
                                                                                <td class="text-center">
                                                                                    <form action="{{ route('analisis.rekomendasi-gap.index') }}" method="GET" target="_blank" style="display:inline;">
                                                                                        ...
                                                                                        <input type="hidden" name="pangkat_jabatan_id" value="{{ request('pangkat_jabatan_id','') }}">
                                                                                        <input type="hidden" name="jenis_ptk_id" value="{{ request('jenis_ptk_id','') }}">
                                                                                        <input type="hidden" name="kota_id" value="{{ request('kota_id','') }}">
                                                                                        <input type="hidden" name="jenjang_pendidikan_id" value="{{ request('jenjang_pendidikan_id','') }}">
                                                                                ...
                                                                                <td rowspan="{{ count($rek['detail_gap']) }}" class="text-center" style="vertical-align: middle;">
                                                                                    <div class="d-flex align-items-center gap-2">
                                                                                        ...
                                                                                        <div class="progress flex-grow-1" style="height:6px;">
                                                                                            <div class="progress-bar {{ $progressClass }}" role="progressbar" style="width: {{ min($percentage,100) }}%;"></div>
                                                                                        </div>
                                                                                    </div>
                                                                                </td>
                                                                            @endif
                                                        ...
                                                        <tfoot>
                                                            ...
                                                            <tr class="table-info">
                                                                ...
                                                                <td colspan="3" class="text-center fw-bold">{{ count($jenjang['rekomendasi'] ?? []) }}</td>
                                                            </tr>
                                                        </tfoot>
                                                    </table>
                                                </div>
...
<script>
    ...
    function updateAnalisisContent(data) {
            ...
            data.rekomendasi_gap_per_jenjang.forEach((jenjang, idx) => {
                ...
                (jenjang.rekomendasi || []).forEach((rek) => {
                    ...
                    rek.detail_gap.forEach((gap, gapIndex) => {
                        ...
                        const percentage = jenjangTotalPtk > 0 ? Math.round((jumlah / jenjang.total_ptk) * 100 * 10) / 10 : 0;
        ...
        if (data.progress_kota?.length > 0) {
            ...
            data.progress_kota.forEach((kota) => {
                const cls = kota.persentase >= 80 ? 'bg-success' : kota.persentase >= 50 ? 'bg-warning' : 'bg-danger';
        ...
        if (typeof window.__buildAnalisisNav === 'function') {
            try { window.__buildAnalisisNav(); } catch(e) {}
        }
    ...
    function renderCharts(data) {
        ...
        if (levelTerendahCtx && data.level_terendah_per_ptk) {
            levelTerendahChart = new Chart(levelTerendahCtx, {
                type: 'bar',
                data: {
                    labels: data.level_terendah_per_ptk.labels || ['Level 1', 'Level 2', 'Level 3', 'Level 4', 'Level 5'],
                    datasets: [
                        {
                            label: 'Jumlah PTK',
                            data: data.level_terendah_per_ptk.data || [0, 0, 0, 0, 0],
                            backgroundColor: ['rgba(220, 53, 69, 0.8)', '#17a2b8', '#007bff', '#ffc107', '#28a745'],
                            borderColor: ['rgba(220, 53, 69, 0.8)', '#148899', '#0069d9', '#e0a800', '#218838'],
                            borderWidth: 1,
                        },
                    ],
                },
                options: {
                        ...
                        tooltip: {
                            callbacks: {
                                ...
                                afterLabel: (ctx) => {
                                    ...
                                    const names = ['Dasar', 'Penerapan', 'Analisis', 'Evaluasi', 'Pembimbingan'];
                    ...
                    scales: {
                        y: { beginAtZero: true, ticks: { stepSize: 1 }, title: { display: true, text: 'Jumlah PTK' } },
                        x: { title: { display: true, text: 'Level Kompetensi Terendah' } },
                    },
                },
            });
        }
        ...
        if (levelTerendahkabkotaCtx && data.level_kota_per_ptk) {
            levelTerendahkabkotaChart = new Chart(levelTerendahkabkotaCtx, {
                type: 'bar',
                data: {
                    labels: data.level_kota_per_ptk.labels || ['Level 1', 'Level 2', 'Level 3', 'Level 4', 'Level 5'],
                    datasets: [
                        {
                            label: 'Jumlah PTK',
        ...
        if (levelPerKotaCtx && data.distribusi_level_per_kota) {
            levelPerKotaChart = new Chart(levelPerKotaCtx, {
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    scales: {
                        x: { stacked: true, title: { display: true, text: 'Kota' } },
                        y: {
                            stacked: true,
                            beginAtZero: true,
                            ticks: { stepSize: 1 },
                            title: { display: true, text: 'Jumlah PTK' },
                        },
                    },
                    plugins: {
                        legend: {
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            ...
                            title: { display: true, text: 'Nama Pelatihan', font: { size: 13 } },
                        },
                        y: {
                            beginAtZero: true,
                            ticks: { stepSize: 1, font: { size: 11 } },
                            title: { display: true, text: 'Jumlah PTK', font: { size: 13 } },
                        },
                    },
                },
            });
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            ...
                            title: { display: true, text: 'Jumlah PTK', font: { size: 10 } },
                        },
                    },
                },
            });
    ...
    (function(){
        ...
        const NAV_ITEMS = [
            { id: "sec-level-ptk",     label: "grafik Distribusi Level PTK" },
            { id: "sec-jenjang",       label: "grafik Distribusi Jenjang" },
            { id: "sec-level-kota",    label: "grafik Distribusi Level per Kota (Layered Bar)" },
            { id: "sec-ptk-belum",     label: "tabel PTK yang Belum Menjawab Instrumen" },
            { id: "sec-modus",         label: "tabel Modus Level per Kota (Jumlah PTK)" },
            { id: "sec-sub-indikator", label: "grafik Distribusi PTK per Sub Indikator Jumlah PTK" },
            { id: "sec-sub-jenjang",   label: "grafik Distribusi PTK per Sub Indikator per Jenjang Jabatan" },
            { id: "sec-gap",           label: "tabel Rekomendasi Kebutuhan Belajar per Jenjang Jabatan" },
            { id: "sec-pelatihan",     label: "grafik Distribusi Pelatihan yang diperlukan PTK" },
        ];
...
(function () {
  ...
  function openModal(title, bodyHtml) {
    ...
    // IMPORTANT: pastiin area body punya tinggi biar canvas kebaca
    bodyEl.style.height = "calc(100vh - 140px)";
  ...
  function closeModal() {
    if (fsChartInstance) {
      try { fsChartInstance.destroy(); } catch (err) {}
  ...
  document.addEventListener("click", (e) => {
    const banned = e.target.closest("button, a, input, select, textarea, form, .nav-tabs, .nav-link");
    if (banned) return;
    ...
    if (tableResp) openModal(title, tableResp.outerHTML);
    else if (tableOnly) openModal(title, `<div class="table-responsive">${tableOnly.outerHTML}</div>`);
  });

  // expose (opsional, tapi aman)
  window.__openAnalisisFullscreen = openModal;
  window.__closeAnalisisFullscreen = closeModal;
// FULLSCREEN POPUP (TABLE/CHART) - MOVE CANVAS (NO CLONE)
(function () {
  ...
  function openModal(title, bodyHtml) {
    ...
    // bikin body punya tinggi fix biar canvas bisa full
    bodyEl.style.height = "calc(100vh - 140px)";
    bodyEl.style.minHeight = "520px";

    document.body.style.overflow = "hidden";
  }

  function restoreCanvas() {
    if (!activeCanvas || !originalParent) return;

    // balikin ke tempat semula
    if (originalNext && originalNext.parentNode === originalParent) {
      originalParent.insertBefore(activeCanvas, originalNext);
    } else {
      originalParent.appendChild(activeCanvas);
    }

    // reset style canvas biar balik normal
    activeCanvas.style.width = "";
    activeCanvas.style.height = "";

    // resize chart setelah balik
    if (activeChart && typeof activeChart.resize === "function") {
      setTimeout(() => activeChart.resize(), 50);
    }
  ...
  // --- OPEN TABLE FULLSCREEN ---
  document.addEventListener("click", (e) => {
    // kalau klik di dalam modal, skip biar gak double trigger
    if (e.target.closest("#fsModalAnalisis")) return;
  ...
  // --- OPEN CHART FULLSCREEN (MOVE CANVAS ASLI) ---
  document.addEventListener("click", (e) => {
    // kalau klik di dalam modal, skip
    if (e.target.closest("#fsModalAnalisis")) return;

    const canvas = e.target.closest("canvas");
    if (!canvas) return;

    // filter canvas chart kamu
    const id = canvas.id || "";
    ...
    if (!chart && canvas.id && window[canvas.id]) {
      chart = window[canvas.id]; // fallback jenjangChart_*
    }
    if (!chart) return; // kalau gak ketemu, ya gak bisa fullscreen
    ...
    // simpen posisi asli untuk restore
    activeCanvas = canvas;
    activeChart = chart;
    ...
    openModal(
      title,
      `<div id="fsChartSlot" style="width:100%; height:100%; min-height:520px;"></div>`
    );

    const slot = document.getElementById("fsChartSlot");
    if (!slot) return;

    // pindahin canvas asli ke slot
    slot.appendChild(canvas);

    // bikin canvas ngikutin slot
    canvas.style.width = "100%";
    canvas.style.height = "100%";

    // paksa resize setelah pindah
    setTimeout(() => {
      if (activeChart && typeof activeChart.resize === "function") {
        activeChart.resize();
      }
  ...
  // expose optional (kalau masih mau)
  window.__openAnalisisFullscreen = openModal;
  window.__closeAnalisisFullscreen = closeModal;
})();
