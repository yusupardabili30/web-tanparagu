...
<body>
            ...
            <div class="carousel-container" style="position: relative; z-index: 1;">
                <div class="carousel-content">
                    ...
                    <h1 class="system-name">
                        <img src="{{ asset('build/images/logobgtkPutih.png') }}" class="system-logo"
                ...
                <form id="login-form" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nip" id="nip" class="form-control"
                            ...
                            placeholder="Masukkan 18 digit NIP Anda" required autofocus maxlength="18" pattern="\d{18}"
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            <input type="password" name="token" id="tokenInput" class="form-control"
                                value="{{ old('token', '') }}" placeholder="Masukkan token yang diberikan" required>
                    ...
                    <div class="d-grid mt-4">
                        <button type="submit" class="btn btn-login text-white py-2" id="submitBtn"
                    ...
                    <form id="register-form">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="mm-float">
                                    <select name="id_type" id="reg_id_type" class="form-control" required>
                                        <option value="" selected hidden></option>
                                        <option value="nip">NIP (18 digit)</option>
                                        <option value="nik">NIK (16 digit)</option>
                                    </select>
                                    <label class="mm-label">Pilih Identitas *</label>
                                </div>
                            </div>

                            <div class="col-md-6 mb-3 d-none" id="reg_id_wrap">
                                <div class="mm-float">
                                    <input type="text" name="id_number" id="reg_id_number" class="form-control"
                                        placeholder=" " required inputmode="numeric" autocomplete="off">
                                    <label class="mm-label" id="reg_id_label">Nomor Identitas *</label>
                                </div>
                                <small class="form-text text-muted mt-1" id="reg_id_help"></small>
                            </div>

                            <!-- NUPTK -->
                            <div class="col-md-6 mb-3">
                                <div class="mm-float">
                                    <input type="text" name="nuptk" class="form-control" placeholder=" "
                                        maxlength="16">
                            ...
                            <div class="col-12 mb-3">
                                <div class="mm-float">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="sekolah_option"
                                            id="optionSekolah" value="sekolah" checked>
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="mm-float">
                                    <select name="jenjang_pendidikan_id" id="jenjangSelect" class="form-control"
                                        required>
                                        <option value="" selected hidden></option>
    ...
    <div class="modal fade" id="searchSekolahModal" tabindex="-1" aria-labelledby="searchSekolahModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        // ============================================
        // 1.6 IDENTITAS (NIP/NIK) - REGISTER MODAL  ✅ FIX: GLOBAL (Bukan di dalam handler lain)
        // ============================================
        function initRegisterIdentity() {
            const typeSel = document.getElementById('reg_id_type');
            const wrap = document.getElementById('reg_id_wrap');
            const input = document.getElementById('reg_id_number');
            const label = document.getElementById('reg_id_label');
            const help = document.getElementById('reg_id_help');

            if (!typeSel || !wrap || !input || !label || !help) return;
            ...
            const apply = () => {
                ...
                if (t === 'nip') {
                    label.textContent = 'Nomor Induk Pegawai (NIP) *';
                    help.textContent = 'Masukkan 18 digit NIP. Contoh: 197010021990092010';
                    input.setAttribute('maxlength', '18');
                    input.setAttribute('pattern', '\\d{18}');
                    input.value = (input.value || '').replace(/\D/g, '').slice(0, 18);
                } else {
                    label.textContent = 'Nomor Induk Kependudukan (NIK) *';
                    help.textContent = 'Masukkan 16 digit NIK (sesuai KTP).';
            ...
            // restrict hanya angka + limit sesuai type
            input.addEventListener('input', () => {
                const t = typeSel.value;
                let v = (input.value || '').replace(/\D/g, '');
                const max = (t === 'nik') ? 16 : 18;
        ...
        document.getElementById('login-form')?.addEventListener('submit', function(e) {
            ...
            fetch("{{ route('lockscreen.authenticate') }}", {
                ...
                .then(async (response) => {
                    // ✅ FIX: kalau server balikin HTML/500, jangan langsung response.json() meledak tanpa pesan
                    let data = null;
                    try {
                        data = await response.json();
                    } catch (err) {
                        throw new Error('RESP_NOT_JSON');
                    }
                    return data;
                })
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.show_register_modal) {
                            document.getElementById('modal-info-text').innerHTML =
                                `Identitas <strong>${data.nip}</strong> belum terdaftar dalam sistem. Silakan lengkapi data diri Anda.`;

                            document.getElementById('reg_kegiatan_id').value = data.kegiatan_id;
                            document.getElementById('reg_token').value = data.token;

                            // default: dari server masih "nip"
                            document.getElementById('reg_id_type').value = 'nip';
                            document.getElementById('reg_id_number').value = (data.nip || '');
                            initRegisterIdentity();
        ...
        // ============================================
        // 5. REGISTER FORM  ✅ FIX: validasi id_type/id_number dipindah ke dalam handler click
        // ============================================
        ...
        document.getElementById('submit-register')?.addEventListener('click', function() {
            ...
            // required general (yang kamu punya)
            const requiredFields = ['nama', 'jenis_kelamin', 'tempat_lahir', 'tgl_lahir', 'pangkat_jabatan_id', 'email', 'no_hp', 'agama', 'jenis_ptk_id'];
            let isValid = true;
            let errorMessages = [];
            ...
            const idType = document.getElementById('reg_id_type');
            const idNumber = document.getElementById('reg_id_number');

            if (!idType.value) {
                isValid = false;
                errorMessages.push('Pilih Identitas (NIP/NIK)');
                idType.classList.add('is-invalid');
            }

            if (!idNumber.value.trim()) {
                isValid = false;
                errorMessages.push('Isi Nomor Identitas');
                idNumber.classList.add('is-invalid');
            } else {
                const digits = idNumber.value.replace(/\D/g, '');
                const needLen = (idType.value === 'nik') ? 16 : 18;
                if (digits.length !== needLen) {
                    isValid = false;
                    errorMessages.push(`Nomor ${idType.value.toUpperCase()} harus ${needLen} digit`);
                    idNumber.classList.add('is-invalid');
                }
            }
            ...
            Swal.fire({
                    ...
                    <div class="text-start">
                        ...
                        <div class="border p-2 mt-2 rounded bg-light">
                            <small>
                                ...
                                • ${idType.value.toUpperCase()}: ${document.getElementById('reg_id_number').value}<br>
                ...
                if (result.isConfirmed) {
                    ...
                    // ✅ set field backend sesuai pilihan
                    const digits = idNumber.value.replace(/\D/g, '');
                    formData.delete('nip');
                    formData.delete('nik');
                    formData.set(idType.value, digits); // nip atau nik
                    formData.set('id_type', idType.value); // optional
