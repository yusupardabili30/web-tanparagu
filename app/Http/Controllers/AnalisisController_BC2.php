...
class AnalisisController extends Controller
{
    ...
    {
        // Set sql_mode konsisten antara lokal dan server untuk semua method
    ...
    private function getAnalisisData(Request $request)
    {
        ...
        if ($request->filled('kegiatan_id') && $request->filled('jenis_ptk_id')) {
            ...
            $ptkYangSudahMenjawabQuery = DB::table('ptk_jawaban')
                ...
                ->where('level', '>=', 1)
                ->where('tahap', 2)  // filter hanya tahap 2
                ->groupBy('ptk_id');
            ...
            $jenjangPendidikanDistribution = DB::table('ptk')
                ...
                ->leftJoin('jenjang_pendidikan', 'ptk.jenjang_pendidikan_id', '=', 'jenjang_pendidikan.jenjang_pendidikan_id')
                ->whereIn('ptk.ptk_id', $ptkIds)
                ->whereNotNull('ptk.jenjang_pendidikan_id') // ← tambahkan ini
                ->whereNotNull('jenjang_pendidikan.jenjang_pendidikan') // ← dan ini
    ...
    private function getRekomendasiGapPerJenjang(Request $request)
    {
        ...
        foreach ($sortedJenjangList as $jenjang) {
            ...
            // ============================================================
            // OPTIMASI UNTUK DATA JUTAAN - PAKAI AGREGASI SQL LANGSUNG
            // ============================================================

            // STEP 1: Ambil ringkasan per PTK (level maksimum per sub_indikator)
            // Ini mengurangi data dari jutaan ke (jumlah_ptk * jumlah_sub_indikator)
            ...
            // STEP 2: Ambil semua rekomendasi dari ptk_jawaban_rekomendasi untuk kegiatan ini
            ...
            // STEP 3: Proses data tanpa load ke memory besar
            ...
            foreach ($groupedBySubIndikator as $subIndikatorId => $subData) {
                ...
                // Hitung distribusi level per PTK
                $ptkByLevelDicapai = [];
                ...
                // Sort dan bersihkan duplicate gaps
                ...
                // Hapus duplicate level_gap untuk kombinasi level_dicapai yang sama
                $uniqueGaps = [];
                foreach ($detailGap as $gap) {
                    $key = $gap['level_dicapai'] . '_' . $gap['level_gap'];
                    if (!isset($uniqueGaps[$key])) {
                        $uniqueGaps[$key] = $gap;
                    }
                }
